...
It is **product-neutral**: nothing here assumes a particular project, company, or toolchain. It is useful to anyone whose agent writes reports, updates, strategy docs, review findings, handoffs, or recommendations for a human reader. In this pack it is the communication gate that Quality Strategy Skills (QSS) runs before a generated strategy, test strategy, tooling strategy, review, or variant is considered finished — but it does not depend on QSS.
