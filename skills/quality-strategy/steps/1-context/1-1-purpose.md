...
- [ ] When the file was created this sub-step, the header carries the version stamp with `<version>` resolved from `$PLUGIN_ROOT/.claude-plugin/plugin.json` (a real version number, not the literal `<version>`).
- [ ] When the file was created this sub-step, the Part 1 heading carries the revisable-basis line — boilerplate, written once, never reworded per project.
...
*Release: <the release this strategy covers — the user's working name for it if one exists yet, else "to be named at Step 2"; confirmed and stamped at sub-step 2.1>*
*Generated by the `quality-strategy` skill — quality-strategy-skills (tollens-ai) v<version> · github.com/tollens-ai/quality-strategy-skills*
...
*This context is a working basis, not fixed. Later steps may surface that changing the team, org, or workflow itself — not the product — is the more efficient path to the goals; that comes back as a Part 7 process-change action, not a silent edit here.*
...
**The version stamp.** Replace `<version>` with the `version` field read from `$PLUGIN_ROOT/.claude-plugin/plugin.json` **at generation time** — read the field now and substitute the value; never copy a literal version number out of this prose (it would rot). This is the plugin/release version, the single source of truth, so the stamp can never drift from the code that produced the strategy. It is what lets an alpha bug report against this document be traced back to the exact skill version that generated it. Write the stamp once, when you create the file; on a later revision it is refreshed to the current version (see SKILL.md → Revision mode). The stamp is deliberate attribution — like a document footer — and the presentation-cleanup scans leave it alone.
