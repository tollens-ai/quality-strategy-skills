...
Then offer the Step 7 choice — it is optional (see SKILL.md → "The plan of work is a sketch"): *"Step 7 sketches the plan of work — what to do about these gaps, classified and sequenced, at one or two lines per action. It's optional: if you're going straight into the follow-on lanes — `/test-strategy`, `/oracle-strategy`, `/process-strategy` (builds via `/tooling-strategy`) — we can record the plan as deferred to those instead and close the strategy out now. Sketch it, or defer?"* If the user defers, follow the deferral path in SKILL.md (write the Part 7 deferral note, then the closing moves). If they sketch, proceed to sub-step 7.1.
