...
A gap, a hot item, a below-bar actual — each says *this behaviour is wrong*. Before you let it stand as a defect, ask the upstream question: **what does this behaviour protect?** Behaviour exists for reasons, and a thing that looks like a bug through one dimension is often a deliberate choice serving another. The chess-platform case: *"the clock doesn't pause on disconnect → players get flagged unfairly → angry players"* was named a defect — but a running-on-disconnect clock is the **domain norm** for chess sites, because pausing it lets a player disconnect to think for free. Same behaviour, two dimensions pulling opposite ways: fairness-to-the-disconnected vs anti-cheat integrity. Goal-tracing it to the stated *"angry players"* dealbreaker and stopping there names one side as a bug and never sees the other.
