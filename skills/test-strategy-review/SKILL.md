...
>     - **(iv) scratch-file path citations** — a `quality/.scratch/<…>.md` path listed in a "Sources consulted" block, or as what a learning need or allocation row "rests on". Cite the real underlying source instead (the risk-map row, the named file, the tooling-adequacy *finding*) or drop it — `.scratch/` is working state the reader does not have. **This is the exact leak the test-strategy leg previously had no backstop for**: a transient `.scratch/3.x-…` path in a "Sources consulted" line is caught here now, not only by an external grep.
>
> One exemption: a `## Since the last cycle` section is content, not machinery — its what-happened verdicts and newly-found needs compare the project against its prior strategy, which the reader wants; leave it.
>
> **Severity:** FAIL on checks 1–3, 13, and 14 is a blocker. The rest — including check 15's leak FLAGs — are flags (matching the quality-strategy leg, where the equivalent leak scan is FLAG-severity).
>
> Output format: a markdown list of the fifteen checks with PASS/FLAG/FAIL, one-line explanation, suggested fix where applicable, and meta-note for FAIL/FLAG cases.
