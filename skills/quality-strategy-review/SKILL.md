...
> 11. **Actions classified.** Every Part 7 action is classified as testing / stakeholder / fixing. If Part 7 is a **recorded deferral** — a short section explicitly deferring the plan of work to the follow-on lanes and naming where each slice will live (`/test-strategy`, `/oracle-strategy`, `/process-strategy`, builds via `/tooling-strategy`) — checks 11 and 12 PASS automatically. A Part 7 that is missing, empty, or vague *without* that explicit deferral note is a FAIL as before.
...
- Risk map **dominated by Unknowns whose to-resolve notes say nothing can judge them yet** → recommend **`/oracle-strategy`** first (with `/tooling-strategy` planning agreed builds): decide how the project will judge those dimensions, then `/test-strategy` once the means of judging exist.
