...
> 18. **Strategy job stated.** There is a `## Strategy job` paragraph near the top naming the strategy's job (durable production / pre-implementation / agentic one-shot / lightweight slice, or the user's framing), what evidence it must produce, and what's deliberately out of scope. FAIL if absent.
...
- Any oracle FAIL in subagent A (includes a missing `## Strategy job` paragraph (check 18) and a missing scratch file for a claimed dispatch (check 20) — the latter is hard evidence of a fabricated dispatch).
