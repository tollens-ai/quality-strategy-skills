...
If `quality/archive/` holds a prior version of the strategy (`strategy-*.md`), this is a **revision review**: the most recent archived version is the review's instrument for the revision checks — subagent A diffs it against the current doc (check 22) rather than trusting the revision's own account of what changed.
...
> - **Revision look-forward passes (revision runs only)** — if the strategy is a revision (a prior version under `$PROJECT_DIR/quality/archive/`, or revision markers in the doc), expect `revision-defect-recon.md` and `revision-context-scan.md` — the deliberately-blind look-forward dispatches. Missing when the revision re-asserted the risk map's actuals is a FAIL; an explicit skip note in the doc (the edit made no claim about the project's current state) stands in.
>
