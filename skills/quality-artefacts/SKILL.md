...
- **The hero stat.** Every strategy contains one staggering honest number — *"the #1 churn driver: zero measurements, ever"*, *"1,122 tests pass — 0 cover what killed the alpha."* Find it and set it in poster type. Honest-but-jaw-dropping is the house move; the Wrapped feeling comes from framing, never from inflation.
...
A caution from experience: the classic **quality radar** (dimensions as spokes, required vs actual polygons) is the most template-shaped of all forms — every project's radar looks alike, which undercuts the feel-seen test. Reach for it only when the holes in the map are themselves the story and you can make them dramatic voids, not side annotations. Also derivable: per-stakeholder cards ("what does *good* mean for the agents?"), a before/after gap tracker across two strategy revisions, a one-dimension deep-dive for a decision meeting.
