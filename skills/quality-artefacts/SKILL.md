...
- **Look at the thing.** If a headless browser is available (Chromium via `--headless --screenshot=<png> --window-size=<WxH>` is common), render the artefact and *read the screenshot yourself* — overflowing text, collisions, and a wrong first impression are visible in a render and invisible in markup. Fix what you see and re-render. If no renderer exists, say so in your summary and ask the user to glance before sharing.
