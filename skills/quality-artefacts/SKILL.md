...
- **Render integrity** *(scores under principle 2's gate)*. Declare the encoding in every form the artefact ships: HTML carries `<meta charset="utf-8">`; a raw SVG opens with an explicit XML declaration (`<?xml version="1.0" encoding="UTF-8"?>`) — a browser loading an `.svg` directly does not assume UTF-8. At review, open the artefact every way a user plausibly would — from `file://`, served, the `.svg` loaded directly — and look for mojibake or glyph breakage. *Shipped bug: em-dashes and apostrophes rendered as "â€™ / â€”" because an SVG shipped without a charset declaration.*
