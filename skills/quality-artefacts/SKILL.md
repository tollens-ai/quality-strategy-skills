...
Two finishing rules that recur: a **final summary grid** (the all-dimensions appendix frame) gets colour — state-tinted tiles, not white cards with coloured words; and any form not listed here is the skill working as intended. Also derivable: per-stakeholder cards ("what does *good* mean for the agents?"), a before/after gap tracker across two strategy revisions, a one-dimension deep-dive for a decision meeting.
