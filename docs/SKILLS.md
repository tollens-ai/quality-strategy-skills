...
| `/evaluation-strategy` | The **evaluation lane** (formerly `/oracle-strategy`) — same light shape: filter for the ilities where better quality instruments would make the project knowable (Unknowns nothing can judge yet, claims resting on sand, ilities tracked only by an unbounded proxy), then have/improve/add per ility — oracles (trusted to judge) and proxies (indicate, with known blind spots) both. Hands agreed builds to `/tooling-strategy`. |
...
| `/tooling-strategy` | The build plan for "how do we know?". Gathers everything the lanes couldn't answer — risk-map Unknowns nothing can judge yet, `/evaluation-strategy`'s agreed builds, `/test-strategy`'s blocked questions — into one prioritised oracle/instrument build plan. The most re-runnable doc in the pack. |
| `/strategy-variants` | Post-processing. From a finished, reviewed `quality/strategy.md`, derive audience-facing variants without touching the original: a distributable one-pager and a client-safe ("polite") version pitched at an external reader the user names — never an invented one. Gates mechanically on strategy completeness (Parts 1/3/4/6 present, else stop and point at `/quality-strategy`). Omits and re-pitches; never asserts quality the strategy doesn't support. |
...
| `/oracle-adequacy` | The "how do we know?" audit for actual-state claims — are the oracles behind a strategy's actuals trustworthy? Offered from `/evaluation-strategy` when trust is contested, or standalone. Shares its oracle taxonomy with `/tooling-adequacy`. |
